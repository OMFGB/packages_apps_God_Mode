...
public class OMGBBroadcastReceivers extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
           Constants.LOCAL_FILE = "/sdcard/t3hh4xx0r/downloads/omgb/" + Constants.getDeviceScript();
   	   Slog.d(TAG, "Received");
	   //First lets start with the bulletin board posts
	   downloadOMGB(context);
	}

	private void downloadOMGB(Context context) {
	...
	Downloads.refreshOMGB();
   }

   public void getOMGBValues() {
	File local = new File (Constants.LOCAL_FILE);
	...
	if (!Constants.LOCAL_MD5.equals(Constants.REMOTE_MD5)) {
	    ...
	    Slog.d(TAG, "Your OMGB Builds are up to date.");
	}
    ...
    public void alertUser(Context context) {
        Slog.d(TAG, "You have new OMGB builds.");
		 ...
		 CharSequence contentTitle = "OMGB Builds";  // expanded message title
		 CharSequence contentText = "New OMGB Build available.";      // expanded message text

		 Intent notificationIntent = new Intent(context, com.t3hh4xx0r.addons.omgb.OMGBActivity.class);
